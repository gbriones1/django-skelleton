...
from settings.forms import EmailsForm, ReportsForm
...
def main(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'reports':
            if not config:
                ...
                config[0].week_cut = request.POST.get('week_cut', 4)
                config[0].save()
    ...
    if config:
        emails_form = EmailsForm(instance=config[0])
        reports_form = ReportsForm(instance=config[0])
